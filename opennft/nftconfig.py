...
class LegacyNftConfig(BaseModel):
    ...
    dicom_first_image_nr: int = Field(alias='DicomFirstImageNr')
    ...
    weights_file_name: str = Field(alias='WeightsFileName')
    mc_template_file: Path = Field(alias='MCTempl')
    struct_bg_file: Path = Field(alias='StructBgFile')
    task_dir: Optional[Path] = Field(alias='TaskFolder')
    prot: str = Field(alias='Prot')
    offline_mode: bool = Field(alias='OfflineMode')
    use_tcp_data: bool = Field(alias='UseTCPData')
    tcp_data_address: Optional[IPv4Address] = Field(alias='TCPDataIP')
    tcp_data_port: Optional[int] = Field(alias='TCPDataPort')
    type: str = Field(alias='Type')
    max_feedback_val: int = Field(alias='MaxFeedbackVal')
    feedback_val_dec: int = Field(alias='FeedbackValDec')
    neg_feedback: bool = Field(alias='NegFeedback')
    plot_feedback: bool = Field(alias='PlotFeedback')
    use_ptb: bool = Field(False, alias='UsePTB')
    display_feedback_screen_id: int = Field(0, alias='DisplayFeedbackScreenID')
    display_feedback_fullscreen: bool = Field(False, alias='DisplayFeedbackFullscreen')
    targ_ang: int = Field(alias='TargANG')
    targ_rad: int = Field(alias='TargRAD')
    targ_diam: int = Field(alias='TargDIAM')
