...
class Nfb:
    ...
    def nfb_calc(self, is_rtqa):
        ...
        typeflag = self.session.config.type
        ...
        if typeflag == "PSC" and (self.session.config.prot == "Cont" or self.session.config.prot == "ContTask"):
        ...
        if self.session.config.prot == "Inter" and (typeflag == "PSC" or typeflag == "Corr"):
            ...
            if condition == 2:
                ...
                if first_nf == ind_vol_norm:
                    ...
                    if typeflag == "PSC":
                        ...
                        for i_roi in range(nr_rois):
                            m_bas = np.median(scale_time_series[i_roi][i_block_bas])
                            m_cond = np.median(scale_time_series[i_roi][i_block_nf])
